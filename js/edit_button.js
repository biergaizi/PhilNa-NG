...
function edInsertLink(which, i, defaultValue) {
	...
	if (!edCheckOpenTags(which, i)) {
		var URL = prompt('请输入一个链接' ,defaultValue);
		if (URL) {
...
function edInsertExtLink(which, i, defaultValue) {
	...
	if (!edCheckOpenTags(which, i)) {
		var URL = prompt('Enter the a  link' ,defaultValue);
		if (URL) {
			edButtons[i].tagStart = '<a href="' + URL + '" rel="external nofollow">';
			edInsertTag(which, i);
		}
	}
	else {
...
function edInsertImage(which) {
    myField = document.getElementById(which);
	var myValue = prompt('image url', 'http://');
	if (myValue) {
		myValue = '<img src="' 
				+ myValue 
...
function edInsertVia(which) {
    myField = document.getElementById(which);
	var myValue = prompt('Enter the a source link', 'http://');
	if (myValue) {
